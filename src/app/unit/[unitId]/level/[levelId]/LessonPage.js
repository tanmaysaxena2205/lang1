  ...
  if (isLocked) {
    return (
      <main className="fixed inset-0 bg-[#050505] flex items-center justify-center p-6 text-center z-50">
        <div className="bg-[#0a0a0a] border border-[#ff6600]/30 p-8 md:p-10 rounded-[2.5rem] md:rounded-[3rem] max-w-sm">
          <h2 className="text-3xl md:text-4xl font-black italic uppercase text-[#ff6600] mb-4">LIMIT REACHED</h2>
  ...
  if (finished) {
    return (
      <div className="fixed inset-0 bg-black flex flex-col items-center justify-center text-white p-6 text-center z-50">
        <Trophy className="text-[#ff6600] mb-6 animate-bounce w-16 h-16 md:w-20 md:h-20" />
        <h1 className="text-4xl md:text-5xl font-black italic uppercase mb-2">COMPLETE</h1>
        <p className="text-white/40 mb-10 font-bold tracking-widest text-xs md:text-base uppercase">LEVEL {unitId}-{levelId} MASTERED</p>
        <button
          disabled={isSaving}
          onClick={async () => {
            ...
            setIsSaving(true);
            try {
              const finishedId = `1-${unitId}-${levelId}`;
              await completeSublevel(clerkId, finishedId);
          ...
          className="bg-[#ff6600] text-black px-12 md:px-20 py-4 md:py-5 rounded-2xl md:rounded-3xl font-black text-xl md:text-2xl active:scale-95 transition-transform"
        >
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-[#0a0a0a] text-white overflow-hidden touch-none">
      ...
      <div className="shrink-0 flex items-center gap-4 md:gap-6 px-4 md:px-6 h-16 md:h-20 w-full max-w-5xl mx-auto z-10 bg-[#0a0a0a]">
        ...
        <div className="flex-1 h-2 md:h-3 bg-white/10 rounded-full overflow-hidden border border-white/5">
          <div
            className="h-full bg-[#ff6600] transition-all duration-500 shadow-[0_0_15px_rgba(255,102,0,0.5)]"
            style={{ width: `${((currentIdx + 1) / questions.length) * 100}%` }}
          />
        </div>
        <span className="font-black italic text-[#ff6600] text-sm md:text-lg tabular-nums">
      ...
      <div className="flex-1 w-full max-w-3xl mx-auto flex flex-col items-center justify-start md:justify-center px-4 md:px-6 pt-20 md:pt-0 overflow-y-auto pb-40 md:pb-0">
          ...
          <div className="w-full max-w-[92%] md:max-w-full flex flex-col">
            ...
            <div className="grid gap-3 md:gap-4 w-full mx-auto">
              {current.options.map((opt) => (
                ...
                >
                  <span>{opt}</span>
                  <Volume2 className={`w-5 h-5 md:w-7 md:h-7 ${selected === opt ? "text-black" : "text-white/20"}`} />
                </button>
              ))}
            </div>
          </div>
        ...
        {current.type === "jumble" && (
          <div className="w-full max-w-[92%] md:max-w-full flex flex-col items-center gap-6 md:gap-8">
            ...
            <div className={`min-h-[100px] md:min-h-[140px] w-full border-2 flex flex-wrap gap-2 md:gap-3 justify-center items-center p-6 md:p-8 bg-white/[0.03] rounded-[1.5rem] md:rounded-[2.5rem] transition-all ${status === 'wrong' ? 'border-red-500/50' : 'border-white/10'}`}>
              {jumbledOrder.map((item, i) => (
                <button
                  key={`${item.id}-${i}`}
                  onClick={() => setJumbledOrder(prev => prev.filter((_, idx) => idx !== i))}
                  className="bg-[#ff6600] text-black px-4 py-2 md:px-6 md:py-3 rounded-lg md:rounded-xl font-black text-base md:text-xl shadow-lg active:scale-90"
                >
                  {item.word}
                </button>
              ))}
              ...
              {current.options.map((word, i) => {
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-3 md:px-7 md:py-4 border-2 rounded-lg md:rounded-xl font-black text-sm md:text-lg transition-all ${isUsed
      ...
      <div className={`shrink-0 h-24 md:h-28 flex items-center px-4 md:px-6 transition-all duration-300 border-t-2 ${status === 'correct' ? 'bg-green-600 border-green-400' :
        ...
        <div className="max-w-4xl mx-auto w-full flex justify-between items-center">
          <div className="flex items-center gap-3 md:gap-4">
            {status !== 'idle' && (
              <div className="bg-white p-1.5 md:p-2.5 rounded-full text-black">
                {status === 'correct' ? <CheckCircle2 className="w-6 h-6 md:w-8 md:h-8" /> : <AlertCircle className="w-6 h-6 md:w-8 md:h-8" />}
              </div>
            )}
            <div className="flex flex-col">
              ...
              {status === 'wrong' && (
                <p className="text-white/90 font-bold text-[10px] md:text-sm uppercase tracking-widest mt-1">
                  Correct: {current.a}
                </p>
              )}
            </div>
          ...
          <button
            ...
            className="bg-white text-black px-8 md:px-12 py-3 md:py-4 rounded-xl md:rounded-2xl font-black text-lg md:text-xl uppercase transition-all active:scale-95 disabled:opacity-20 shadow-xl"
          >
