...
export default function SignInPage() {
  ...
  const signInWith = async (strategy) => {
    ...
    try {
      ...
      // Clerk throws an error if the user doesn't exist during OAuth/SSO sometimes 
      // or if the strategy fails. We catch it here.
      const isUserNotFound = err.errors?.some(e => e.code === "form_identifier_not_found" || e.code === "user_not_found");
      
      if (isUserNotFound) {
        setError("Your email is not registered. Please sign up first.");
      } else {
        ...
        <div className="bg-neutral-900/50 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-8 md:p-12 shadow-2xl">
          ...
          <div className="text-center mb-10">
            <h1 className="text-3xl font-black text-white italic uppercase tracking-tighter">
              LANG<span className="text-orange-500 underline decoration-white/20">STR</span>
            </h1>
            <p className="text-gray-400 mt-2 font-medium italic">Welcome Back to Langster</p>
          </div>

          {/* ERROR MESSAGE BOX */}
          {error && (
            <div className="mb-6 flex items-center gap-3 p-4 bg-orange-500/10 border border-orange-500/20 rounded-2xl animate-in fade-in slide-in-from-top-2">
              <AlertCircle className="text-orange-500 shrink-0" size={20} />
              <p className="text-sm font-bold text-orange-200 leading-tight">
                {error}
              </p>
            </div>
          )}
          ...
          <div className="space-y-3 mb-8">
            ...
            >
              ...
              <span className="text-white font-bold text-sm">Sign in with Apple</span>
            </button>
          </div>
